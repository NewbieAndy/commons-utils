...
/**
 * @ClassName: CardIdVerifyUtil
 * @Description: 身份证号验证工具类
 * @author: Andy(mch)
 * @date: 2015年6月11日
 * @version: V1.0.0
 */
public class CardIdVerifyUtil {


    /**
     * 身份证号验证方法
     *
     * @param cardIdStr 身份证帐号
     * @return 身份证号码正确返回true, 错误返回false
     */
    public static boolean IDCardValidate(String cardIdStr) {
        String Ai = "";

        //将身份证最后一位的x转换为大写，便于统一  
        cardIdStr = cardIdStr.toUpperCase();

        // 判断号码的长度 15位或18位  
        if (cardIdStr.length() != 15 && cardIdStr.length() != 18) {
            return false;
        }

        // 18位身份证前17位位数字，如果是15位的身份证则所有号码都为数字  
        if (cardIdStr.length() == 18) {
            Ai = cardIdStr.substring(0, 17);
        } else if (cardIdStr.length() == 15) {
            Ai = cardIdStr.substring(0, 6) + "19" + cardIdStr.substring(6, 15);
        }
        ...
        try {
            ...
            //获取当月最大天数
            int maxDaysOfMonth = getMaxDaysByYearAndMonth(strYear, strMonth);
            if (Integer.parseInt(strDay) > maxDaysOfMonth || Integer.parseInt(strDay) == 0) {
                return false;
            }
        ...
        if (isVarifyCode(Ai, cardIdStr) == false) {
            return false;
        }
        return true;
    }
     ...
     * @return
     */
    private static int getMaxDaysByYearAndMonth(String dyear, String dmouth) {
